...
$recherche = null;
    if(Session::getInstance()->contient("requeteFiltreConvention")){
        $recherche = Session::getInstance()->lire("requeteFiltreConvention");
    }
...
?>
    <form method="post" action="controleurFrontal.php?controleur=convention&action=rechercherConvention">
        ...
        <div class="button">
            ...
            <a href="controleurFrontal.php?controleur=convention&action=supprimerFiltreConvention"> Rénitialiser </a>
        </div>
    </form>
    </div>
    </div>
<?php
echo "<div class='page'>";
echo "<div> <a class='' href='controleurFrontal.php?controleur=convention&action=afficherGestionConvention&page=" . $pageActuelle - 1 . "'> page précédente </a> </div>";
if ($pageActuelle != 1) {
    echo "<div> <a class='' href='controleurFrontal.php?controleur=convention&action=afficherGestionConvention&page=" . $pageActuelle - 1 . "'>" . $pageActuelle - 1 . "</a> </div>";
}
echo "<div> <p> $pageActuelle </p> </div>";
if ($pageActuelle != $nbrePages) {
    echo "<div> <a class='' href='controleurFrontal.php?controleur=convention&action=afficherGestionConvention&page=" . $pageActuelle + 1 . "'>" . $pageActuelle + 1 . "</a> </div>";
}
echo "<div> <a class='' href='controleurFrontal.php?controleur=convention&action=afficherGestionConvention&page=" . $pageActuelle + 1 . "'> page suivante </a> </div>";
echo "</div>";

echo '<div class="page">';
echo "<div> <a class='' href='controleurFrontal.php?controleur=convention&action=afficherCreationConvention'> Création d'une convention </a> </div>";
echo '</div>';
